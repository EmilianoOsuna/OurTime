...
function Onboarding({ onComplete }: { onComplete: () => void }) {
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--paper)', position: 'relative' }}>
      {step > 0 && step < 5 && (
        <div style={{ position: 'fixed', top: 'max(44px, calc(var(--sat) + 14px))', left: 24, right: 24, zIndex: 10, display: 'flex', gap: 6 }}>
          {[1, 2, 3, 4].map(i => (
...
function Welcome({ onNext }: { onNext: () => void }) {
  return (
    <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', padding: '0 30px max(40px, calc(var(--sab) + 16px))', position: 'relative' }}>
      <div style={{ flex: 1 }} />
...
function ReadyStep({ name, onComplete }: { name: string; onComplete: () => void }) {
  return (
    <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column',
      alignItems: 'center', justifyContent: 'center', textAlign: 'center', padding: '40px 32px max(40px, calc(var(--sab) + 16px))' }}>
      <Confetti show={true} />
...
function StepShell({ no, eyebrow, title, sub, children }: {
  ...
  return (
    <div className="page-enter" style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', padding: '96px 30px max(38px, calc(var(--sab) + 16px))' }}>
